...
public class BoardController {
    ...
    //update board_tb set title = ?, content = ?, is_pubic = ? where id = ?
    ...
    public @ResponseBody Resp<?> setBoardOne(@PathVariable("id") int id) {
        User sessionUser = (User) session.getAttribute("sessionUser");
    ...
    // localhost:8080
    @GetMapping({"/", "/api/board"}) // 일반적으로 index 페이지는 두 가지 주소를 사용함
    public @ResponseBody Resp<?> list(HttpServletRequest request,
